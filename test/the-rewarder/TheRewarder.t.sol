...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        ...
        // Get proofs for player (index 188)
        bytes32[] memory dvtProof = merkle.getProof(dvtLeaves, 188);
        bytes32[] memory wethProof = merkle.getProof(wethLeaves, 188);

        // Calculate how many claims we need to drain each token
        uint256 dvtRemaining = TOTAL_DVT_DISTRIBUTION_AMOUNT -
            ALICE_DVT_CLAIM_AMOUNT;
        uint256 wethRemaining = TOTAL_WETH_DISTRIBUTION_AMOUNT -
            ALICE_WETH_CLAIM_AMOUNT;
        ...
        // The bug: transfer happens every iteration, but _setClaimed only runs on token change or last element
        ...
        // Transfer everything to recovery
